...
@Composable
fun SimpleLineChart(values: List<Double>, xLabels: List<String>, title: String) { /* unchanged */
    if (values.isEmpty()) {
    ...
    Column {
        ...
        Canvas(modifier = Modifier.fillMaxWidth().height(180.dp).padding(top = 4.dp)) {
            ...
            val points = safeValues.mapIndexed { idx, v -> Offset(leftPad + idx * stepX, 12f + plotH - ((v / max).toFloat() * plotH)) }
