...
fun HomeScreen(vm: FinanceViewModel = hiltViewModel()) {
    ...
    var selectedCategoryId by remember { mutableLongStateOf(state.categories.firstOrNull()?.id ?: 1L) }
            ...
            state.categories.take(5).forEach { category ->
                AssistChip(
                    ...
                    colors = AssistChipDefaults.assistChipColors(
                        containerColor = if (selectedCategoryId == category.id) colorFromHex(category.colorHex).copy(alpha = 0.25f) else MaterialTheme.colorScheme.surfaceVariant
                    )
                )
            }
        }
