...
fun HomeScreen(vm: FinanceViewModel = hiltViewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        SimpleLineChart(state.transactions.take(10).map { it.amount })
        SimpleBarChart(state.transactions.groupBy { it.date.monthValue }.values.map { list -> list.sumOf { it.amount } })
